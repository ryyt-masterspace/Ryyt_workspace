...
export default function TrackingPage() {
    ...
    const getStepStatus = (stepIndex: number) => {
        ...
        else if (status.includes('PROCESS') || status.includes('BANK')) currentLevel = 2;
        else if (status.includes('INITIATED') || status.includes('CREATED')) currentLevel = 1;
        else currentLevel = 0;

        const activeStepIndex = currentLevel - 1;
        ...
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-blue-500/30 flex justify-center p-4 sm:p-8">
            <div className="w-full max-w-md space-y-8">
                <div className="flex items-center justify-center gap-2 opacity-80">
                    {brandLogo ? (
                        <div className="relative h-10 w-full max-w-[160px] mb-4">
...
function TimelineItem({ status, title, desc, date }: { status: string, title: string, desc: string, date?: string }) {
